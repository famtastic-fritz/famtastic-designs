...
line="*/5 * * * * cd $root && /usr/local/bin/php $root/vendor/drush/drush/drush.php php:script $worker >> $base/logs/locs-booking-worker.log 2>&1"
current="$(crontab -l 2>/dev/null || true)"
if [[ "$current" == *"$marker"* ]]; then
  actual="$(printf '%s\n' "$current" | awk -v marker="$marker" '$0==marker {getline; print}')"
  [[ "$actual" == "$line" ]] || { echo 'Existing Locs worker differs; review required.'; exit 5; }
...
[[ "$mode" == --apply ]] || exit 0
umask 077
mkdir -p "$base/logs" "$base/cron-backups"
