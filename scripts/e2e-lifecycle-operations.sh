...
FAMTASTIC_LIFECYCLE_EVIDENCE_DIR="$evidence_dir" \
FAMTASTIC_TRANSACTIONAL_EMAIL_TRANSPORT=memory \
FAMTASTIC_TRANSACTIONAL_EMAIL_CAPTURE="$capture" \
  "$repo_root/backend/vendor/bin/drush" --root="$repo_root/backend/web" php:script "$repo_root/backend/scripts/e2e-lifecycle-operations.php"
test -s "$evidence_dir/evidence.json"
jq -e '.status == "passed" and ([.checks[]] | all)' "$evidence_dir/evidence.json" >/dev/null
echo "PASS: unified lifecycle evidence is complete and every assertion is true."
