...
[[ "$slug" == "and-if-it-is" ]] || { echo "Only the reviewed and-if-it-is Lab slug is allowed by this publisher." >&2; exit 2; }
site_dir="$(cd "$site_dir" && pwd)"
expected_site="$repo_dir/marketing/campaigns/and-if-it-is-rattler-lifers/lab-site"
[[ "$site_dir" == "$expected_site" ]] || { echo "SITE_DIR must be the reviewed AND IF IT IS Lab source." >&2; exit 2; }
...
[[ "$file_count" -le 50 ]] || { echo "Lab page exceeds 50 files." >&2; exit 1; }
[[ "$total_bytes" -le 15728640 ]] || { echo "Lab page exceeds 15 MiB." >&2; exit 1; }

archive="$(mktemp /tmp/famtastic-lab.XXXXXX.tar.gz)"
trap 'rm -f "$archive"' EXIT
COPYFILE_DISABLE=1 tar --no-xattrs -C "$site_dir" -czf "$archive" .
checksum="$(openssl dgst -sha256 "$archive" | awk '{print $NF}')"

echo "FAMtastic Lab publication candidate"
echo "  files:    $file_count"
echo "  bytes:    $total_bytes"
echo "  sha256:   $checksum"
echo "  target:   $public_origin/lab/$slug/"
...
remote_target="$remote_root/lab/$slug"
remote_stage="$remote_root/lab/.staging-$slug"
remote_archive=".config/famtastic-lab-$slug.tar.gz"
...
archive="$3"
replace="$4"
[[ "$target" == "public_html/lab/and-if-it-is" ]] || { echo "Unsafe target" >&2; exit 1; }
[[ "$stage" == "public_html/lab/.staging-and-if-it-is" ]] || { echo "Unsafe staging target" >&2; exit 1; }
[[ "$archive" == ".config/famtastic-lab-and-if-it-is.tar.gz" ]] || { echo "Unsafe archive" >&2; exit 1; }
if [[ -e "$HOME/$target" && "$replace" != true ]]; then
  echo "Lab target already exists; pass --replace for a recoverable atomic revision." >&2
  exit 1
fi
test ! -e "$HOME/$stage" || { echo "Lab staging target already exists." >&2; exit 1; }
if [[ -e "$HOME/$archive" ]]; then
  test -f "$HOME/$archive"
  test ! -L "$HOME/$archive"
fi
mkdir -p "$HOME/public_html/lab"
REMOTE_PREFLIGHT
...
if [[ -e "$HOME/$target" ]]; then
  [[ "$replace" == true ]] || { echo "Existing Lab target requires --replace." >&2; exit 1; }
  backup="$HOME/public_html/lab/.backup-and-if-it-is-$(date -u +%Y%m%dT%H%M%SZ)"
  test ! -e "$backup"
...
echo "PASS: FAMtastic Lab destination published atomically"
echo "URL: $public_origin/lab/$slug/"
