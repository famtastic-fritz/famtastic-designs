...
"$DRUSH" eval "\$db = \Drupal::database(); \$request = \$db->select('famtastic_project_request', 'r')->fields('r')->condition('public_id', '$website_request_id')->execute()->fetchAssoc(); \$notice = \$db->select('famtastic_notification_outbox', 'n')->fields('n', ['template_id', 'template_version', 'body'])->condition('notification_key', 'website-request:' . \$request['id'] . ':customer')->execute()->fetchAssoc(); assert(\$notice['template_id'] === 'customer_intake_submitted'); assert((int) \$notice['template_version'] === 2); assert(str_contains((string) \$notice['body'], '/portal/?section=projects&request=$website_request_id'));"
"$DRUSH" eval "
