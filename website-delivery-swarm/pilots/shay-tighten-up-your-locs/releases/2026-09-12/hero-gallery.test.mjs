...
test('explicit pause stays paused across focus and hover untilresumed',()=>{const s=setup();s.click('rotation');s.root.handlers.mouseenter();s.root.handlers.mouseleave();s.root.handlers.focusout({relatedTarget:null});assert.equal(s.running(),false);s.click('rotation');assert.equal(s.running(),true);});
test('hover and document visibility suspend automatic rotation',()=>{const s=setup();s.root.handlers.mouseenter();assert.equal(s.running(),false);s.root.handlers.mouseleave();assert.equal(s.running(),true);s.doc.hidden=true;s.doc.handlers.visibilitychange();assert.equal(s.running(),false);s.doc.hidden=false;s.doc.handlers.visibilitychange();assert.equal(s.running(),true);});
