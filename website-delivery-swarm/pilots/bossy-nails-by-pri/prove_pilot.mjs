...
const output = resolve(process.argv[2] || join(repo, 'artifacts', 'website-delivery-swarm', 'bossy-nails-by-pri-20260818'));
...
const research = {
  ...
  conducted_at: now,
  scope: 'User-supplied business concept and conversion-pattern synthesis; no claims about a real client.',
  findings: [
    { topic: 'audience', finding: 'The user defined the audience as flashy, new-age Black women on the go.', source_type: 'user-supplied brief', use: 'Primary creative and conversion context.' },
    { topic: 'place', finding: 'The user supplied St. Lucie County, Florida as the service market.', source_type: 'user-supplied brief', use: 'Locale label only; no market-size or operating claim.' },
    { topic: 'conversion', finding: 'A useful proof must keep service discovery, preparation, scope confirmation, and booking handoff visible.', source_type: 'creative synthesis', use: 'Fictional information architecture pending client validation.' },
    { topic: 'creative range', finding: 'The requested mix is exactly one restrained, one medium-FAMtastic, and four ultra-FAMtastic directions.', source_type: 'user-supplied brief', use: 'Hard generation and QA contract.' },
  ],
  unknowns_requiring_client_confirmation: [
    'Real owner identity, credentials, business registration, brand assets, and service area',
    'Exact services, pricing, timing, availability, booking system, address, and contact details',
    'Preparation, sanitation, cancellation, repair, accessibility, privacy, and consent policies',
    'Actual portfolio, testimonials, photography permissions, and product affiliations',
  ],
  prohibited_assumptions: [
    'No real salon affiliation or operating-business claim',
    'No domain availability claim',
    'No invented credential, address, phone number, price, availability, policy, or legal promise',
  ],
};
...
const architecture = {
  site_type: 'five-page appointment-led nail studio website',
  pages: 5,
  primary_conversion: 'Submit a scoped appointment request',
  secondary_conversions: ['Explore service lanes', 'Review nail-art concepts', 'Prepare for an appointment', 'Confirm policies'],
  package: {
...
const routes = [{ id: 'review', path: 'index.html', expectedH1: 'Six sets' }, ...directions.map((direction) => ({
  id: direction.id,
  path: direction.entry || `${direction.slug}/index.html`,
  expectedH1: {
    'direction-a': 'Polished',
    'direction-b': 'Move fast',
    'direction-c': 'Wear your',
    'direction-d': 'Enter the',
    'direction-e': 'Main character',
    'direction-f': 'The Bossy',
  }[direction.id],
}))];
...
const technicalChecks = {
  ...
  exact_requested_mix: directions.filter((direction) => direction.mode === 'restrained').length === 1 && directions.filter((direction) => direction.mode === 'medium').length === 1 && directions.filter((direction) => direction.mode === 'ultra').length === 4,
  four_ultra_famtastic: directions.filter((direction) => direction.mode === 'ultra').length === 4 && directions.filter((direction) => direction.mode === 'ultra').every((direction) => direction.famtastic_level === 10),
...
addTrace({ agent: 'brief-and-market-researcher', provider: 'user-brief-plus-creative-synthesis', model: 'source-synthesis-v1', executionClass: 'local', outputValue: research, assertions: { brief_findings_present: research.findings.length >= 4, no_external_fact_dependency: scenario.research.external_facts_required === false, unknowns_visible: research.unknowns_requiring_client_confirmation.length >= 4, no_real_client_claim: scenario.fictional_business === true } });
...
addTrace({ agent: 'creative-director', provider: 'openai', model: 'codex-session', executionClass: 'cloud', outputValue: directions, assertions: { exact_six: directions.length === 6, restrained_count_one: directions.filter((direction) => direction.mode === 'restrained').length === 1, medium_count_one: directions.filter((direction) => direction.mode === 'medium').length === 1, ultra_count_four: directions.filter((direction) => direction.mode === 'ultra').length === 4, distinct_architectures: technicalChecks.distinct_information_architecture } });
...
console.log('PASS: Bossy Nails by Pri six-proof website delivery swarm');
console.log('PASS: one restrained, one medium, and four ultra-FAMtastic directions');
