...
$flag=$argv[1]??'';$map=['--enable-mail'=>'LOCS_MAIL_ENABLED','--enable-booking'=>'LOCS_PUBLIC_BOOKING_ENABLED','--disable-booking'=>'LOCS_PUBLIC_BOOKING_ENABLED'];
...
$old=file_get_contents($path);$key=$map[$flag];$value=$flag==='--disable-booking'?'false':'true';
$next=preg_replace('/^'.preg_quote($key,'/').'="(?:true|false)"$/m',$key.'="'.$value.'"',$old,-1,$n);if($n!==1)throw new RuntimeException('flag_not_unique');
