...
const GROUPS = [
  ['Workspace', [['home', 'Home'], ['projects', 'Projects'], ['services', 'Services']]],
  ['Help', [['messages', 'Messages'], ['support', 'Support'], ['faq', 'FAQs']]],
  ['Growth', [['grow', 'Growth ideas'], ['referrals', 'Referrals']]],
  ['Account', [['billing', 'Billing'], ['settings', 'Settings'], ['account', 'Account']]],
];
...
function PortalServices({ workspace, catalog, go, compact = false }) {
  ...
  return <section className={`portal-service-hub${compact ? ' compact' : ''}`} aria-labelledby={compact ? 'portal-services-preview-title' : 'portal-services-title'}>
    ...
    {!compact && <section className="portal-intake-share-box" style={{ marginTop: '1.75rem', padding: '1.25rem', borderRadius: '14px', border: '1px dashed rgba(124,252,0,0.35)', background: 'rgba(124,252,0,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}><div><span style={{ color: '#7cfc00', fontSize: '0.75rem', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.1em' }}>⚡ Direct Project Intakes</span><h4 style={{ margin: '0.2rem 0', fontSize: '1.05rem', color: '#fff' }}>Need to share a specialized intake form with a partner or team?</h4><p style={{ margin: 0, color: '#9da79d', fontSize: '0.84rem' }}>Share tailored intake forms for Hosting Setup, AI Chatbots, Custom Portals, or Ongoing Site Care.</p></div><Link to="/intake" style={{ padding: '0.65rem 1.25rem', borderRadius: '9px', background: '#7cfc00', color: '#000', fontWeight: '800', textDecoration: 'none', fontSize: '0.85rem' }}>Open Intake Hub →</Link></section>}
...
export default function CustomerPortalDashboard() {
  ...
  const initialSection = requestedTab && Object.hasOwn(LABELS, requestedTab) ? requestedTab : (continuingWebsiteLead ? 'projects' : 'home');
  ...
  const proofReady = (request) => [3, 6].includes(request?.proofs?.variants?.length);
  const activeRequest = useMemo(() => {
    const requests = workspace?.website_requests || [];
    if (!requests.length) return null;
    return requests.find((request) => request.public_id === activeRequestId)
      || requests.find((request) => ['customer_ready', 'notified'].includes(request.proof_review_status) && proofReady(request))
      || requests[0];
  }, [workspace, activeRequestId]);
  const requestChips = (workspace?.website_requests || []).length > 1 ? (
    <div className="portal-request-chips" role="tablist" aria-label="Website requests">
      {workspace.website_requests.map((request) => (
        <button key={request.public_id} role="tab" aria-selected={request.public_id === activeRequestId}
          className={request.public_id === activeRequestId ? 'active' : ''}
          onClick={() => setActiveRequestId(request.public_id)}>
          <strong>{request.project_name}</strong>
          <small>{title(request.status)}{proofReady(request) ? ' · concepts ready' : ''}</small>
        </button>
      ))}
    </div>
  ) : null;
  ...
  return <div className={`portal-app ${menu ? 'menu-open' : ''}`}>
    ...
    <main className="portal-main">
      ...
      {section === 'projects' && <>
        ...
        {editingRequest && <Panel key={editingRequest.public_id || 'new-request'} id="website-request-editor" eyebrow={editingRequest.public_id ? 'Continue request' : 'New website request'} title={editingRequest.project_name || 'Tell us what you want to build'} className="portal-request-form"><form onSubmit={saveWebsiteRequest}>
          {!editingRequest.public_id && <p className="portal-form-stepnote"><strong>Step 1 of 2.</strong> Answer three quick things and save your draft — the full interview opens right after, and everything saves as you go.</p>}
          <div className="portal-form-grid"><label>Request name<input name="project_name" defaultValue={editingRequest.project_name || ''} placeholder="Example: Sweet Crumbs Bakery website" required /></label><label>What are we building?<select name="project_type" defaultValue={editingRequest.project_type || 'new_website'}><option value="new_website">New website</option><option value="landing_page">Landing page</option><option value="redesign">Website redesign</option><option value="online_store">Online store / shopping cart</option></select></label></div>
          ...
          <fieldset className="portal-form-group"><legend>Goals and customers</legend>
          ...
          <fieldset className="portal-form-group"><legend>Your business</legend>
            <label>Business name<input name="business_name" defaultValue={editingRequest.business_name || ''} autoComplete="organization" /></label>
          ...
          <fieldset className="portal-form-group"><legend>Pages, content, and features</legend>
            ...
            <label>Ongoing maintenance needs<textarea name="maintenance_needs" defaultValue={editingRequest.intake?.maintenance_needs || ''} /></label>
          </fieldset>
          <fieldset className="portal-form-group"><legend>Brand and design direction</legend>
          ...
          <fieldset className="portal-form-group"><legend>Domains, email, and access</legend>
            <label>Domain plan<select name="domain_choice" defaultValue={editingRequest.domain_choice || 'undecided'}><option value="undecided">I’m not sure yet</option><option value="new_domain">I need a new domain</option><option value="existing_domain">I already own a domain</option></select></label>
          ...
          <fieldset className="portal-form-group"><legend>Store, booking, AI, and wrap-up</legend>
            ...
            <label>Booking or appointment details<textarea name="booking_details" defaultValue={editingRequest.intake?.booking_details || ''} /></label>
            <label>AI agent goals<textarea name="ai_agent_goals" defaultValue={editingRequest.intake?.ai_agent_goals || ''} /></label>
            <fieldset><legend>Optional AI brief enrichment</legend><label>Connection mode<select name="ai_enrichment_mode" defaultValue={editingRequest.intake?.ai_enrichment_mode || 'none'}><option value="none">No AI enrichment</option><option value="famtastic_managed">Use FAMtastic-managed models</option><option value="customer_managed">I want to connect my own provider later</option></select></label><label>Context for the AI reviewer<textarea name="ai_context_notes" defaultValue={editingRequest.intake?.ai_context_notes || ''} placeholder="Do not paste API keys or passwords." /></label><label className="portal-check"><input name="life_path_opt_in" type="checkbox" defaultChecked={editingRequest.intake?.life_path_opt_in} />Use optional life-path guidance only for voice and creative suggestions.</label></fieldset>
            <div className="portal-form-grid"><label>Budget context (optional)<textarea name="budget_context" defaultValue={editingRequest.intake?.budget_context || ''} placeholder="This does not lock you into a package; it helps us recommend the smallest useful solution." /></label><label>Anything else Fritz should know<textarea name="notes" defaultValue={editingRequest.intake?.notes || ''} /></label></div>
          ...
          <div className="portal-form-actions portal-sticky-actions"><button name="action" value="save" disabled={busy}>{busy ? 'Saving…' : editingRequest.public_id ? 'Save draft' : 'Save draft & open the full brief'}</button>{editingRequest.public_id && <button className="secondary" name="action" value="submit" disabled={busy}>Submit for review</button>}<button className="quiet" type="button" onClick={() => setEditingRequest(null)}>Close</button></div>
        ...
        <section className="portal-request-list">{requestChips}{!activeRequest && <Panel eyebrow="Projects" title="No website requests yet"><p>Start with a short brief — your concepts, proofs, and purchase live here.</p></Panel>}{activeRequest ? <Panel key={activeRequest.public_id} id={`website-request-${activeRequest.public_id}`} tabIndex={activeRequest.public_id === targetRequest ? -1 : undefined} className={activeRequest.public_id === targetRequest ? 'portal-request-target' : ''} eyebrow={activeRequest.status === 'converted' ? 'Purchased project request' : 'Website request'} title={activeRequest.project_name}><dl><div><dt>Status</dt><dd>{title(activeRequest.status)}</dd></div><div><dt>Proofs</dt><dd>{title(activeRequest.proof_review_status)}</dd></div><div><dt>Updated</dt><dd>{date(activeRequest.changed)}</dd></div></dl>{activeRequest.private_offer && <div className="portal-private-offer-card" style={{ margin: '1rem 0', padding: '1.15rem', border: '1px solid #7cfc00', borderRadius: '14px', background: 'rgba(124,252,0,0.08)' }}><span style={{ color: '#7cfc00', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: '800' }}>⚡ Exclusive Private Offer Active</span><h3 style={{ margin: '0.4rem 0 0.2rem', fontSize: '1.25rem' }}>{activeRequest.private_offer.reason || 'Special Approved Package Price'}</h3><p style={{ margin: '0.25rem 0 0.85rem', color: '#c2ccc2' }}>Offered Rate: <strong style={{ color: '#7cfc00', fontSize: '1.2rem' }}>{money(activeRequest.private_offer.offered_amount_minor)}</strong>{activeRequest.private_offer.list_amount_minor > activeRequest.private_offer.offered_amount_minor && <span style={{ textDecoration: 'line-through', opacity: 0.6, marginLeft: '0.65rem' }}>{money(activeRequest.private_offer.list_amount_minor)} list</span>}</p><button type="button" onClick={() => navigate(`/buy?request=${encodeURIComponent(activeRequest.public_id)}`)}>Accept Offer & Purchase →</button></div>}{activeRequest.intake?.recommendation && <p><strong>Recommended path: {activeRequest.intake.recommendation.label}</strong><br /><small>{activeRequest.intake.recommendation.reasons?.join(' ')}</small></p>}{[3, 6].includes(activeRequest.proofs?.variants?.length) && <WebsiteProofReview request={activeRequest} busy={busy} onDecision={decideProof} onShare={shareProof} />}{!activeRequest.proofs && activeRequest.status === 'submitted' && <p>{activeRequest.proof_review_status === 'owner_review' ? 'Your concepts are complete and in FAMtastic quality review. We’ll notify you when the complete set is approved.' : 'Your brief is in the studio queue. We’ll notify you when your working concepts are ready.'}</p>}{!['converted', 'cancelled'].includes(activeRequest.status) && <><button onClick={() => setEditingRequest(activeRequest)}>Continue request</button>{activeRequest.direct_checkout_available && <button className="secondary" onClick={() => navigate(`/buy?request=${encodeURIComponent(activeRequest.public_id)}`)}>Purchase {activeRequest.intake?.recommendation?.label}</button>}</>}{activeRequest.status === 'converted' && <p>This request is connected to its paid project below.</p>}</Panel> : null}</section>
      ...
      {section === 'grow' && <section className="portal-grid three">{workspace.offers.map((offer) => <Panel key={offer.key} eyebrow="Recommended next step" title={offer.title} className="offer"><p>{offer.description}</p><small>Recommended from your current services and project stage.</small><button onClick={() => go('services')}>Explore options →</button></Panel>)}<Panel eyebrow="Not sure what you need?" title="Have FAMtastic handle it" className="lime"><p>Tell us the outcome you want. We’ll recommend the smallest useful next step.</p><button onClick={() => go('support')}>Describe my goal</button></Panel></section>}
