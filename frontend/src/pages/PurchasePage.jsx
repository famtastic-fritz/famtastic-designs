...
export default function PurchasePage() {
  ...
  async function checkout(event) {
    ...
    try {
      const result = await createCommerceCheckout({ organization: organization.public_id, skus: [BASE_SKU, ...selected], domain_choice: domainChoice, recurring_authorized: renewal, accept_terms: terms, terms_version: state.terms.version, marketing_opt_in: marketing });
  ...
  return <form className="purchase-shell" onSubmit={checkout}>
    <span>Secure Commerce checkout</span><h1>{base?.title || 'Web Basics Bundle'}</h1><p>{base?.summary}</p>
