...
export default function PurchasePage() {
  ...
  const customScopePending = websiteRequest && !baseSku;
  ...
  // Renewal price comes from the server catalog via the renewal SKU - the
  // legally required consent must never show a hardcoded number (audit #5).
  const renewalSku = (state.products || []).find((item) => item.sku === (base?.billing?.renewal_sku || ''));
  const renewalPrice = renewalSku ? money(renewalSku.price) : 'the published';
  return <form className="purchase-shell" onSubmit={checkout}>
    ...
    {customScopePending && <div className="purchase-context" role="status"><strong>Your brief needs a custom quote.</strong> FAMtastic is preparing scoped pricing for your request — you will receive it by email and in your portal. Nothing is charged while you wait.</div>}
    {(customScopePending || !websiteRequest) && <fieldset><legend>Choose the package that fits</legend>{websiteBundles.map((item) => <label key={item.sku}><input type="radio" name="bundle" value={item.sku} checked={baseSku === item.sku} onChange={(event) => setBaseSku(event.target.value)} required /> <b>{item.title}</b> — {money(item.price)}<small>{item.summary}</small></label>)}</fieldset>}
    ...
    <label className="purchase-consent"><input type="checkbox" checked={renewal} onChange={(e) => setRenewal(e.target.checked)} required /> I authorize the hosting included with this package to renew at {renewalPrice}{renewalSku ? '/month' : ''} rate after the included first year. I can cancel before renewal from my account.</label>
