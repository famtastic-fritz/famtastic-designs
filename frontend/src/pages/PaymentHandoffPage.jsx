...
export default function PaymentHandoffPage() {
  ...
  const opened = () => fetch(`${prefix}/api/payment-handoff/${encodeURIComponent(organization)}/${encodeURIComponent(siteKey)}/events`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ event: 'opened', surface: 'starter' }) });
  return <main className="purchase-shell"><span>Client-owned payment handoff</span><h1>{h.label}</h1>{h.qr_image_url && <img src={h.qr_image_url} alt="Payment QR code" />}<p>{h.instructions}</p>{h.destination_url && <a className="btn btn--lime" href={h.destination_url} target="_blank" rel="noreferrer" onClick={opened}>{h.label}</a>}<p><small>{h.destination_url ? 'Opening this link is not a payment confirmation, order, or service reservation.' : 'Scan the QR code to pay directly. Scanning is not a payment confirmation, order, or service reservation.'}</small></p></main>;
}
