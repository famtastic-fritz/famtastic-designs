  ...
  return <details className="portal-form-group">
    ...
    {pages.map((page, index) => <fieldset key={index}>
      ...
      {[['page_name', 'Page name'], ['title', 'Browser title'], ['heading', 'Page heading'], ['description', 'Short page description'], ['body', 'Page text']].map(([field, label]) => <label key={field}>
        {label}
        {field === 'body' ? <textarea name={`page_copy_${field}`} value={page[field] || ''} onChange={event => update(index, field, event.target.value)} />
          : <input name={`page_copy_${field}`} value={page[field] || ''} required={field === 'page_name'} onChange={event => update(index, field, event.target.value)} />}
      </label>)}
