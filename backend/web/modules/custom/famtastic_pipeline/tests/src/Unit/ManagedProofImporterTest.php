...
final class ManagedProofImporterTest extends ManagedProofImportFixture {
  public function testAtomicImportRetainsUnknownBillingAndOriginalsWithoutQaOrNotice(): void {
    ...
    foreach ($this->rows('proof_variant') as $v) {
      ...
      self::assertArrayNotHasKey('design_dna', $v); self::assertNull($v['design_dna__format']);
      $dna = json_decode($v['design_dna__value'], TRUE); self::assertArrayHasKey('worker_description', $dna);
  ...
  public function testFixtureMatchesDrupalTextLongTableMapping(): void {
    $field = $this->createMock(\Drupal\Core\Field\FieldStorageDefinitionInterface::class);
    $field->method('getName')->willReturn('design_dna');
    $field->method('isBaseField')->willReturn(TRUE);
    $field->method('isMultiple')->willReturn(FALSE);
    $field->method('getColumns')->willReturn(\Drupal\text\Plugin\Field\FieldType\TextLongItem::schema($field)['columns']);
    $entity = $this->createMock(\Drupal\Core\Entity\ContentEntityTypeInterface::class);
    $entity->method('getBaseTable')->willReturn('proof_variant');
    $mapping = new \Drupal\Core\Entity\Sql\DefaultTableMapping($entity, ['design_dna' => $field]);
    self::assertSame(['value' => 'design_dna__value', 'format' => 'design_dna__format'], $mapping->getColumnNames('design_dna'));
    self::assertFalse($this->db->schema()->fieldExists('proof_variant', 'design_dna'));
    foreach ($mapping->getColumnNames('design_dna') as $column) self::assertTrue($this->db->schema()->fieldExists('proof_variant', $column));
    self::assertSame('proof-import:job:1', $this->import()['receipt_id']);
  }
  ...
  public function testHistoricalAcknowledgmentAfterSelectionDoesNotWeakenFreshBindingOrGrantReads(): void {
    ...
    $this->db->update('proof_campaign')->fields(['selected_variant' => 'b'])->execute();
