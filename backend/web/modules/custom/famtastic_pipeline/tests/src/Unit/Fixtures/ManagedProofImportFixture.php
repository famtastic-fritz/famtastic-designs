...
abstract class ManagedProofImportFixture extends UnitTestCase {
  ...
  protected function setUp(): void {
    ...
    $this->db->query('CREATE TABLE proof_campaign (id INTEGER PRIMARY KEY AUTOINCREMENT, campaign_id TEXT, prospect_id INTEGER, business_name TEXT, status TEXT, generation_status TEXT, studio_job_id TEXT, expires_at INTEGER, ready_at INTEGER, selected_variant TEXT, changed INTEGER)');
    $this->db->query('CREATE TABLE proof_variant (id INTEGER PRIMARY KEY AUTOINCREMENT, campaign_id INTEGER, direction_id TEXT, direction_name TEXT, artifact_path TEXT, thumbnail_path TEXT, preview_url TEXT, design_dna__value TEXT, design_dna__format TEXT, created INTEGER)');
    ...
    foreach (['proof_campaign' => ProofCampaign::class, 'proof_variant' => ProofVariant::class] as $table => $class) {
      ...
      $storage->method('create')->willReturnCallback(function (array $values) use ($table, $class) {
        ...
        $entity->method('save')->willReturnCallback(function () use ($values, $table, &$id) {
          $columns = $values;
          if ($table === 'proof_variant') {
            // Match the real text_long SQL mapping, not the entity input shape.
            self::assertSame(['value', 'format'], array_keys($columns['design_dna']));
            $columns['design_dna__value'] = $columns['design_dna']['value'];
            $columns['design_dna__format'] = $columns['design_dna']['format'];
            unset($columns['design_dna']);
          }
          $id = (int) $this->db->insert($table)->fields($columns)->execute();
