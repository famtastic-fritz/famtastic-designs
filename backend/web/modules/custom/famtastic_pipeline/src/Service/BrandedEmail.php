...
/** Single approved presentation shell. Content must be escaped by its renderer. */
final class BrandedEmail {
  public const LOGO_URL = 'https://famtasticdesigns.com/brand/famtastic-designs-logo-v1.png';

  public static function render(string $subject, string $paragraphs, string $logoUrl = self::LOGO_URL, string $headline = '', string $badge = 'FAMtastic Designs', string $url = '', string $ctaLabel = '', string $preheader = '', bool $localPreview = FALSE): string {
    if (!($localPreview && $logoUrl === './assets/famtastic-designs-logo-v1.png')
      ...
      throw new \InvalidArgumentException('email_hosted_logo_required');
    }
    $escape = static fn(string $s): string => htmlspecialchars($s, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
    $headline = $escape($headline ?: $subject);
    $preheader = $escape($preheader ?: $subject);
...
<div style="font-size:10px;font-weight:bold;letter-spacing:2px;color:#52613d;margin-bottom:12px;">{$badge}</div>
<h1 class="headline" style="overflow-wrap:anywhere;word-break:break-word;font-size:34px;line-height:1.12;letter-spacing:-1.1px;margin:0 0 25px;color:#111710;">{$headline}</h1>
<div style="font-size:16px;line-height:1.65;color:#252925;overflow-wrap:anywhere;word-break:break-word;">{$paragraphs}</div>
