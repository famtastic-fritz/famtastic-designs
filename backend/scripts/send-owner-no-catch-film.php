...
/**
 * Sends the one owner-authorized No-Catch film notice through the exact-key
 ...
 *
 * Drush: drush php:script /private/path/send-owner-no-catch-film.php [-- --send]
 */

use Drupal\famtastic_pipeline\Service\OutreachMailer;

const NO_CATCH_FILM_NOTICE_KEY = 'no-catch-film-20260919-owner-v1';
const NO_CATCH_FILM_RECIPIENT = 'fritz.medine@gmail.com';
const NO_CATCH_FILM_SUBJECT = 'The new original $199 Special film is ready';
const NO_CATCH_FILM_BODY = "Hi Fritz,\n\nThe new original $199 Special film is ready. It pairs fully original motion graphics with new narration generated locally from the supplied script.\n\nView the film and page:\nhttps://famtasticdesigns.com/why-famtastic/\n\nAlways FAMtastic,\nShay";
const NO_CATCH_FILM_TEMPLATE = 'standard';
const NO_CATCH_FILM_TEMPLATE_VERSION = 2;
const NO_CATCH_FILM_CATEGORY = 'operational';
const NO_CATCH_FILM_MAX_ATTEMPTS = 1;
...
$recipient = NO_CATCH_FILM_RECIPIENT;
$subject = NO_CATCH_FILM_SUBJECT;
$body = NO_CATCH_FILM_BODY;
$template = NO_CATCH_FILM_TEMPLATE;
$templateVersion = NO_CATCH_FILM_TEMPLATE_VERSION;
$notificationKey = NO_CATCH_FILM_NOTICE_KEY;
...
$expectedHash = $bindingHash(
  $notificationKey,
  NO_CATCH_FILM_CATEGORY,
  $recipient,
  $subject,
  $body,
  $template,
  $templateVersion,
  NO_CATCH_FILM_MAX_ATTEMPTS,
);
...
if ($existing) {
  ...
  if ($status !== 'queued'
    ...
    || (int) ($existing['max_attempts'] ?? 0) !== NO_CATCH_FILM_MAX_ATTEMPTS
...
if (!$existing) {
  ...
  try {
    ...
    $db->insert('famtastic_notification_outbox')->fields([
      'notification_key' => $notificationKey,
      'category' => NO_CATCH_FILM_CATEGORY,
      'recipient' => $recipient,
      'subject' => $subject,
      'body' => $body,
      'template_id' => $template,
      ...
      'status' => 'queued',
      'attempts' => 0,
      'max_attempts' => NO_CATCH_FILM_MAX_ATTEMPTS,
      'available_at' => $now,
      'created' => $now,
      'changed' => $now,
    ])->execute();
