...
/**
 * Sends the one owner-authorized female-voice audition notice through the
 * exact-key outbox. Default mode is read-only; pass only --send after its live
 * audio has been independently verified. Never run a broad notification worker.
 *
 * Drush: drush php:script /private/path/send-owner-narration-auditions.php [-- --send]
 */

use Drupal\famtastic_pipeline\Service\OutreachMailer;

const VIDEO_REVIEW_NOTICE_KEY = 'natural-narration-auditions-20260919-v1';
const VIDEO_REVIEW_RECIPIENT = 'fritz.medine@gmail.com';
const VIDEO_REVIEW_SUBJECT = 'Two female voices for your FAMtastic ad';
const VIDEO_REVIEW_BODY = "Hi Fritz,\n\nI made two short local female-voice auditions for the V2 ad: Heart and Bella. Both keep the delivery brisk and add deliberate pauses after the question, between reassurance lines, and around You grow. We grow.\n\nCompare the voices on your phone:\nhttps://famtasticdesigns.com/video-review/#female-voices\n\nThese are stock synthetic voices generated on this Mac with no provider charge. The reusable narration skill and phrase-pause controls are in place. Please listen for the voice you prefer and the fam-TAS-tik pronunciation before we choose the full-ad narrator.\n\nAlways FAMtastic,\nShay";
const VIDEO_REVIEW_TEMPLATE = 'standard';
const VIDEO_REVIEW_TEMPLATE_VERSION = 2;
const VIDEO_REVIEW_CATEGORY = 'operational';
const VIDEO_REVIEW_MAX_ATTEMPTS = 1;
...
$recipient = VIDEO_REVIEW_RECIPIENT;
$subject = VIDEO_REVIEW_SUBJECT;
$body = VIDEO_REVIEW_BODY;
$template = VIDEO_REVIEW_TEMPLATE;
$templateVersion = VIDEO_REVIEW_TEMPLATE_VERSION;
$notificationKey = VIDEO_REVIEW_NOTICE_KEY;
...
$expectedHash = $bindingHash(
  $notificationKey,
  VIDEO_REVIEW_CATEGORY,
  $recipient,
  $subject,
  $body,
  $template,
  $templateVersion,
  VIDEO_REVIEW_MAX_ATTEMPTS,
);
...
if ($existing) {
  ...
  if ($status !== 'queued'
    ...
    || (int) ($existing['max_attempts'] ?? 0) !== VIDEO_REVIEW_MAX_ATTEMPTS
...
if (!$existing) {
  ...
  try {
    ...
    $db->insert('famtastic_notification_outbox')->fields([
      'notification_key' => $notificationKey,
      'category' => VIDEO_REVIEW_CATEGORY,
      'recipient' => $recipient,
      'subject' => $subject,
      'body' => $body,
      'template_id' => $template,
      ...
      'status' => 'queued',
      'attempts' => 0,
      'max_attempts' => VIDEO_REVIEW_MAX_ATTEMPTS,
      'available_at' => $now,
      'created' => $now,
      'changed' => $now,
    ])->execute();
