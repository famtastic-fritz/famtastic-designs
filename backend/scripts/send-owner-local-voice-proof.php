...
/**
 * Sends the one owner-authorized local voice research notice through the
 * exact-key outbox. Default mode is read-only; pass only --send after the report
 * and linked destination have been independently verified. Never run a broad worker.
 *
 * Drush: drush php:script /private/path/send-owner-local-voice-proof.php [-- --send]
 */

use Drupal\famtastic_pipeline\Service\OutreachMailer;

const LOCAL_VOICE_PROOF_NOTICE_KEY = 'video-v2-20260919-local-voice-proof-complete';
const LOCAL_VOICE_PROOF_RECIPIENT = 'fritz.medine@gmail.com';
const LOCAL_VOICE_PROOF_SUBJECT = 'Hear the local voice-conversion test';
const LOCAL_VOICE_PROOF_BODY = "Hi Fritz,\n\nThe free local voice-conversion test ran on your Mac. The comparison plays the original synthetic narrator, a different synthetic reference voice, and the converted result. This is an experimental tool demonstration, not a clone of your personal voice or a finished ad voice.\n\nWatch and listen:\nhttps://famtasticdesigns.com/media/films/local-voice-proof-20260919.mp4\n\nThe final conversion took 7.7 seconds for a 13.4-second sample. OpenVoice V2 ran locally with no paid creative-provider request. An earlier conversion confused the brand closing line in automated transcription; the final run transcribed it correctly. Listening quality still needs review, so I kept this experimental voice out of the finished ads. Your voice match still needs a confirmed recording and your listening review. Voice conversion does not generate walking footage or synchronize new lip movements.\n\nResearch, measured results, and limitations:\nhttps://github.com/famtastic-fritz/famtastic-designs/blob/main/docs/marketing/local-video-studio/VOICE-CLONING-RESEARCH-2026-09-19.md\n\nAlways FAMtastic,\nShay";
const LOCAL_VOICE_PROOF_TEMPLATE = 'standard';
const LOCAL_VOICE_PROOF_TEMPLATE_VERSION = 2;
const LOCAL_VOICE_PROOF_CATEGORY = 'operational';
const LOCAL_VOICE_PROOF_MAX_ATTEMPTS = 1;
...
$recipient = LOCAL_VOICE_PROOF_RECIPIENT;
$subject = LOCAL_VOICE_PROOF_SUBJECT;
$body = LOCAL_VOICE_PROOF_BODY;
$template = LOCAL_VOICE_PROOF_TEMPLATE;
$templateVersion = LOCAL_VOICE_PROOF_TEMPLATE_VERSION;
$notificationKey = LOCAL_VOICE_PROOF_NOTICE_KEY;
...
$expectedHash = $bindingHash(
  $notificationKey,
  LOCAL_VOICE_PROOF_CATEGORY,
  $recipient,
  $subject,
  $body,
  $template,
  $templateVersion,
  LOCAL_VOICE_PROOF_MAX_ATTEMPTS,
);
...
if ($existing) {
  ...
  if ($status !== 'queued'
    ...
    || (int) ($existing['max_attempts'] ?? 0) !== LOCAL_VOICE_PROOF_MAX_ATTEMPTS
...
if (!$existing) {
  ...
  try {
    ...
    $db->insert('famtastic_notification_outbox')->fields([
      'notification_key' => $notificationKey,
      'category' => LOCAL_VOICE_PROOF_CATEGORY,
      'recipient' => $recipient,
      'subject' => $subject,
      'body' => $body,
      'template_id' => $template,
      ...
      'status' => 'queued',
      'attempts' => 0,
      'max_attempts' => LOCAL_VOICE_PROOF_MAX_ATTEMPTS,
      'available_at' => $now,
      'created' => $now,
      'changed' => $now,
    ])->execute();
