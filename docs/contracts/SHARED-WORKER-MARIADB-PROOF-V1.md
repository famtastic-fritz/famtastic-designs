...
September 21, 2026. **Frozen candidate verified on disposable MariaDB; not deployed.**

## Executed integration receipt

Main executed harness `9ad0181c5ac6b5918a084b727bce6d1a67fe4f52` against frozen
production source `1ac3bc26bfc5c60462bf48a9556336880d2b556e`. PHP 8.5.9,
MariaDB 10.11.19-MariaDB-ubu2204, actual REPEATABLE-READ isolation:

- `worker-mariadb-pipes-fixed.znQD8R`: **16 cases / 103 checks**, exit 0,
  `status: pass`, 115.080 seconds guarded wall time. Independent PDO connections
  5–36; three actual root-lock waits each exceeded 35 seconds.
- `worker-mariadb-baseline.YdgBqY`: **four expected negative cases / eight checks**,
  child exit 1, `status: expected_negative_failures`, 1.833 seconds guarded wall
  time. Exact tags: `ownership_escaped`, `stale_active_admitted`, `budget_over_stop`,
  `renewal_overwritten`. No bootstrap/timeout failure substituted for a regression.
- Both guarded runs report protected Studio inventories unchanged. The wrapper
  retains full commands, output and before/after inventories under
  `/tmp/famtastic-phase2-review.NVAfPl/`.
- Exact-owned cleanup completed for all three allocations. The successful run
  `/var/folders/4z/76l8zpns7hvdlykrk2fkf8wr0000gn/T/famtastic-worker-mariadb-Mj1XUG`
  records confirmed absence of container and dedicated network, then deletion of
  temporary connection credentials. Journals/receipts remain. No persistent
  volume or existing service was changed.

This establishes the frozen coordinator's real-database contention boundary,
not installed Drupal migration, asset-writer contention, paid calls, a creative
adapter, import, customer delivery, cloud execution or laptop independence.
Fetched agency `main` moved from f5bc140e to 7227ceb6 during verification; new
private-review work is preserved and separately reconciled before any release.
Historical preparation/failure notes below are retained, not current status.
