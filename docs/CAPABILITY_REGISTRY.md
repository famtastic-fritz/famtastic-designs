...
| Social publishing orchestration | **Never publish-proven — zero campaign posts have ever gone out (verified 2026-09-03).** All five channels (IG/X/TikTok/YouTube/FB) connection-proven 2026-08-25 (tokens verified live; TikTok sandbox w/ daily re-auth); draft→schedule executor locally proven 2026-08-25 (`publish-executor.php`, synthetic-only). 2026-09-03: gates opened to a single env arming switch, stale-date guard added, and campaign-agnostic runner `queue-campaign-drops.py` shipped — **all unproven against a live send.** Sending remains dependent on the operator workstation (Postiz on `127.0.0.1`); see `docs/marketing/CAMPAIGN_POSTING_ARCHITECTURE.md` | Approval, account-specific copy, media publishing, delivery verification, retry evidence, and campaign attribution |
