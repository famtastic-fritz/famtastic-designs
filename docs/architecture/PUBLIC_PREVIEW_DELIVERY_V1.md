...
→ website_proof.generate.v1, research, three real Safe/Wild/OMG proofs
...
→ account-owned refinement, selection, scope, and checkout
```

## Guardrails

- A public room is never created from an image-free pilot, fixture, mock, or chat-only page. Its campaign must contain exactly `a/b/c` Safe/Wild/OMG and its immutable Build DNA projection must be registered before staging.
