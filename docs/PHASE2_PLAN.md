# FAMtastic Designs — Phase 2 Plan
...
- **Keys:** `backend/phase2-setup.sh` generates a 2048-bit RSA keypair with
  `openssl` into `backend/private/` (outside the `backend/web/` docroot;
...
- **`backend/Dockerfile`** — multi-stage: `composer:2` build stage
  ...
  both). Includes `backend/private/` keys via build-time copy or mounted volume.
...
- **`docker-compose.yml`** (repository root) — services: `backend` (build
...
- **`.platform.app.yaml`** (repository root) — Platform.sh single-app definition:
