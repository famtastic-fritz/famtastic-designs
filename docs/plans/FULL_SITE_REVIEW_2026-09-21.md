...
## Nested-path routing correction — 2026-09-21

Production CUA found a real boundary missing from direct service/controller tests: the homepage worked, but nested page and document paths returned Drupal's themed 404. The real route provider rejected the original slash-consuming wildcard before Symfony matching. Customer and staff readers now have explicit one-, two- and three-segment routes with the same access requirements and GET/HEAD methods. The controller reconstructs the original path; the package contract rejects deeper paths. Existing package/source hashes and URL strings are unchanged.

`FullSiteReviewRoutingTest` persists the actual YAML through Drupal's MatcherDumper into SQLite and queries RouteProvider before Symfony matching. It reproduces the original exclusion and covers all supported depths, queries, methods, access declarations, traversal and unsupported depth. Production attachment and release details, including honest initial browser limitations, are in `TRAVEL-ADDICTS-DELIVERY-2026-09-21.md`. Exact private account destinations are kept in the private receipt, not the public repository.
