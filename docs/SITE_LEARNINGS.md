...
Live receipt: the exact four-file publication at `b033218d` preserved agency
homepage, asset and release-marker hashes. Browser navigation and a matching
HTTP hash are separate checks; both passed. Keep the source SHA in later
evidence-only commits, and do not rerun the expected-absent publisher merely to
record documentation. See `docs/plans/ANDERLYE_SHOWCASE_2026-09-21.md`.
...
- **What happened:** The exact published VTT bytes returned `text/plain` on GoDaddy. **Root cause:** The host's default extension mapping did not include WebVTT. **Rule:** Keep a scoped `AddType text/vtt .vtt` rule in the canonical frontend film directory, release it through the normal frontend deployment, and verify the public MIME and a loaded caption track before delivery.
...
Requests 8/9 mapped to prospects 21/30, so email recipients and continuation links were verified through Drupal rather than inferred from the remembered number. Both authorized follow-ups are saved in the original portal conversations and SMTP-accepted once; the initial Travel Addicts identity gap was subsequently resolved through customer/user 6. Use Shay/Shay-Shay and confirm the actual scoped discounted quote later without inventing an amount.
...
Actual signed-in portal/admin acceptance proves the inbox frontend at `fea57649` and final backend at `e68b89f2`, including one authorized clarification saved in the same conversation. Native screenshots confirmed persisted search values even when sanitized browser inspection omitted them. Archive checks must cover both workflow status and `customer_archived_at`; production update metadata can be refreshed separately from cron. Exact release, local test boundaries and SMTP receipts are in `plans/client-messaging-proof-rescue/release.md`.
