...
| 5 | Publish + verify delivery per post | Social Ops | Post ID captured; delivery verified (existing FB verification pattern) | Ledger row per post | ◐ UNPROVEN — first genuine attempt 2026-09-03 09:00 ET, after the OOM-dead orchestrator was revived. **No post has yet been confirmed live on any platform.** Prior status below was written while the publishing worker had been dead since 2026-08-25, so "mechanism ready" was never testable. ◐ MECHANISM READY 2026-08-25 — `backend/scripts/publish-executor.php` converts approved drafts to schedule IN PLACE, verifies QUEUE by read-back, writes evidence; hard double-gated (env + owner flag), idempotent. Not yet run against real drafts. |
