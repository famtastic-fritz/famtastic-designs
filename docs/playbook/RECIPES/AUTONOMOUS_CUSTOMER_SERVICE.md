...
| # | Step | Owner | Definition of done | Evidence | Status |
|---|------|-------|--------------------|----------|--------|
| B1 | Classify inbound into intents (status, revision, billing, technical, other) | Support agent | Classifier rules documented + tested on ≥20 real historical messages | Labeled test set results | ◐ PARTIAL 2026-08-23 — deterministic classifier shipped (`SupportIntentClassifier`, service `famtastic_pipeline.support_intent_classifier`); rules documented in `RUNBOOKS/B1-intent-classification-rules.md`; validated on 27-case labeled corpus (100% intent accuracy, 100% safe-route incl. 9 escalations): `.artifacts/support-triage/1787521055/evidence.json`. REMAINING: re-validate on ≥20 REAL historical messages — needs Fritz-run prod export (exact command prepared at bottom of the runbook). Synthetic-only validation does not close this step. |
...
- 2026-08-23 — Support Triage agent HIRED (`.opencode/agent/fam-support-triage.md`, ROSTER updated) → Phase B started. B1 ◐ PARTIAL: `SupportIntentClassifier` service + rules runbook + unit tests + `scripts/e2e-intent-classifier.sh` validator; 27-case labeled corpus passes at 100% accuracy / 100% safe-route. Real-history validation (≥20 prod messages) BLOCKED on Fritz-run export — command prepared in `RUNBOOKS/B1-intent-classification-rules.md`.
