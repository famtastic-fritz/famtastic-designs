...
|---|------|-------|--------------------|----------|--------|
| 1 | Import & dedupe backlog into catalog register | Portfolio Mgr | `docs/products/CATALOG.md` created: every idea = row w/ source, one-line outcome, suggested price | Register committed | ◐ PARTIAL 2026-08-24 — CATALOG.md created; backlog rows 1–106 imported+scored with dedupe rules stated (batch 1 = rows 1–50 by @fam-portfolio-manager; batch 2 = rows 51–106 appended inline by CEO while subagent dispatches are on hold). Remaining ~1,200 rows queued for batch import (explicitly recorded, not silently skipped). All 106 KILL so far; waves still drawn from owned capabilities. |
