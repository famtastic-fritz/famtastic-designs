...
| 4 | Select wave (3–5) | CEO + Fritz sign-off | Wave chosen by score × strategic fit (feeds the $199/55¢ ladder) | Wave table in MASTER-PLAN weekly report | ◐ STAGED 2026-08-23 — wave proposed: FAM-BUSINESS-EMAIL, FAM-MAINTENANCE, FAM-LOCAL-SEO (all already defined in catalog w/ deal terms; CATALOG.md rationale). Awaiting Fritz sign-off — his 'run T5 to completion' order treated as prep authorization, NOT launch approval. |
| 5 | Run NEW_PRODUCT.md per product | Owners per that recipe | Steps 1–10 complete per product | Links to receipts | ◐ 2026-08-23 — steps 1–10 packets complete for all three (`docs/products/business-email-setup.md`, `website-maintenance-care-plan.md`, `local-seo-setup.md`); commerce validator green for all 16 SKUs incl. these. Only gap inside 1–10: step-3 Stripe TEST price IDs need Fritz's dashboard/API action (exact commands in each packet). Step 11 = GATE per product. |
