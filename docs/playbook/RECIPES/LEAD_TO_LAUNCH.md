...
| 3 | Offer presentation ($199 / 55¢) | System | Correct pricing shown per account scope; package selection is authoritative | Pricing surface test vs Stripe/Commerce catalog | ⚠️ auditor to verify parity |
...
| 7 | **Proof-ready notification to customer** | Mail pipeline | Customer receives proof-ready email; delivery confirmed; failure retries + owner alert | Delivery log visible in `/admin/famtastic` for synthetic event | ⚠️ **BROKEN — first real customer never got this email. Root cause + fix = Standing Order 1. No visibility into sent mail today.** |
| 8 | Customer picks template / selects proof | Portal | Selection recorded; state machine advances; **owner notified instantly**; client gets confirmation of what changed | Event log + two notifications for synthetic pick | ⚠️ **MISSING — selection/edit currently notifies nobody. Standing Order 2.** |
