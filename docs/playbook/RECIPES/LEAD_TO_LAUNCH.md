...
| C6 | Dead code: `PreviewRunnerCallbackController` imports two non-existent classes; `FamtasticPreviewRunnerClient` stranded (MED) | Delete or complete the preview-runner callback stack | unassigned | ⚠️ not started (unreachable today; zero routes reference it) |
| C7 | GA4 covers 2 pages, 2 events; no purchase/lead conversion events (LOW) | Extend `trackEvent` coverage incl. purchase on order-confirm | unassigned | ⚠️ not started |
