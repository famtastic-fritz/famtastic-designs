...
`node_modules/`, `dist/`, and logs. Verified: tracked application files contain none of
these.

`git status` was clean for the application. (The only pending changes in the wider repo were
...
scripts/stripe-setup.sh                       # creates product + $199 price → prints STRIPE_PRICE_ID
