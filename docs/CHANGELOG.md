...
- Opened the gates per owner directive: removed the `--i-have-owner-publish-approval` second gate from `backend/scripts/publish-executor.php` (still accepted, now ignored, so existing runbooks keep working) and opened all 68 record-level approval gates plus `public_publish_enabled` in `marketing/campaigns/55-cents-17-day/manifest.json`. `FAMTASTIC_MARKETING_PUBLISH=true` is now the single arming switch and remains un-defaulted in every committed file.
- Added a **stale-date guard** to `publish-executor.php`. Postiz preserves a post's stored date across the status change, so converting the days 1–3 drafts — still dated 2026-08-23 — would have published twelve backdated posts instantly rather than scheduling them. Such records now report `BLOCKED (stale_date)`; `--allow-stale-dates` overrides deliberately.
- Retired the per-campaign-script pattern that caused blocker 4. `scripts/queue-campaign-drops.py` posts **any** campaign from a single `posting-schedule.json` conforming to the new `marketing/engine/schemas/posting-schedule.schema.json` — no new code per campaign. Supports `--dry-run` (validates media, channels and times without contacting Postiz), idempotent adoption by `utm_content`, per-campaign channel and copy overrides, and read-back verification of every conversion.
- Added `scripts/new-campaign.py` to scaffold and validate a campaign schedule (blocks on TODO placeholders, unmapped channels, offset-less times, duplicate content IDs, and out-of-order drops).
- Added `scripts/retime-campaign-drafts.py` to move existing drafts to the times a manifest declares, verifying by read-back rather than trusting the provider response.
- Re-timed both campaigns per owner directive: 17-day day 1 anchored at 23:59 ET 2026-09-03 with a 2.5h cadence (02:29 / 04:59 / 07:29), and the 4-drop evaluation sequence at 23:50 ET then every 2.5 hours (02:20 / 04:50 / 07:20). Day-1 records now carry an absolute `scheduled_at_et`, because a bare `HH:MM` cannot express a cadence that crosses midnight.
- Reworked `scripts/campaign-readiness.py`, which previously asserted the gates were closed and would have failed forever once they opened. An open manifest must now name its single arming switch and prove that switch is not armed in committed configuration.
- Documented the send-path design flaw and options in `docs/marketing/CAMPAIGN_POSTING_ARCHITECTURE.md`: sending currently depends on the operator Mac being awake with colima and ngrok up, and moving the trigger to Drupal cron alone does not fix it because Postiz itself lives on that machine. Recommendation recorded; the hosting decision remains open.
- Renamed `evaluation-schedule-tonight.json` to `posting-schedule.json` and removed a dangling `<<<<<<< HEAD` merge marker committed into both site-learnings files by `ac212d4`.
