...
<template>
  <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-18">
    <div class="max-w-3xl">
      ...
      <p class="mt-5 text-base leading-8 text-white/72">This page shows the live offer ladder, price framing, and CTA paths without pretending automated payment is already active.</p>
    </div>
    ...
    <div class="mt-10 grid gap-4 lg:grid-cols-3">
      <article v-for="plan in packages" :key="plan.packageName" :class="plan.highlighted ? 'border-[#79FF00]/35 bg-[#79FF00]/8' : 'border-white/8 bg-[#0D1210]'" class="rounded-[28px] border p-7">
        ...
        <p v-if="plan.paymentOptions?.length" class="mt-4 text-xs leading-6 text-white/60">Payment options are finalized during consultation: {{ plan.paymentOptions.join(' • ') }}</p>
