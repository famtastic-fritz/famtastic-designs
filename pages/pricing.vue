...
<template>
  <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-18">
    <div class="max-w-3xl">
      ...
      <h1 class="mt-3 text-4xl font-black text-white">Website packages built around clear outcomes.</h1>
      <p class="mt-5 text-base leading-8 text-white/72">Choose a starting lane below. Final scope is confirmed after a short consultation so the project fits your goals, timeline, and budget.</p>
    </div>
    ...
    <div class="mt-10 grid gap-4 lg:grid-cols-3">
      <article v-for="plan in packages" :key="plan.packageName" :class="plan.highlighted ? 'border-[#79FF00]/35 bg-[#79FF00]/8' : 'border-white/8 bg-[#0D1210]'" class="rounded-[28px] border p-7">
        ...
        <p v-if="plan.paymentOptions?.length" class="mt-3 text-xs leading-6 text-white/60">Payment options are confirmed after consultation: {{ plan.paymentOptions.join(' • ') }}</p>
    ...
    <div class="mt-12 rounded-[28px] border border-white/8 bg-white/[0.03] p-8">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <p class="text-sm text-white/60">Payment options are confirmed after consultation. PayPal-supported payment options are available when project terms are approved.</p>
      </div>
